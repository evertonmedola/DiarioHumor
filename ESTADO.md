...
| --------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------- |
| Lista de registros (todos)                          | AsyncStorage (chave `@diario-humor:registros`) + estado local em `TelaListagem` (`useState` + `useFocusEffect`)                         | Lido do AsyncStorage ao focar a tela; atualizado após salvar/editar/excluir                                                      | `CalendarioMensal`, `GraficoMensal`               |
| Registros filtrados do mês                          | Estado derivado em `TelaListagem` (calculado com `useMemo` a partir da lista completa + mês selecionado)                                | Recalculado automaticamente quando a lista completa ou o mês selecionado mudam                                                   | `CalendarioMensal`, `GraficoMensal`               |
| Mês selecionado (filtro do calendário e do gráfico) | Estado local em `TelaListagem` (`useState`, padrão = mês atual)                                                                         | Ao clicar nas setas do `SeletorMes`                                                                                              | `CalendarioMensal`, `GraficoMensal`, `SeletorMes` |
| Dia selecionado no calendário                       | Estado local em `TelaListagem` (`useState`, padrão = dia de hoje, se estiver no mês exibido)                                            | Ao tocar em uma célula do `CalendarioMensal`                                                                                     | `CardDoDiaSelecionado`                            |
...
| Registro selecionado (para editar/ver)              | Parâmetro de navegação (`route.params.registroId`) + buscado do AsyncStorage no `useEffect` de `TelaFormulario`/`TelaDetalhes`          | Ao tocar no `CardDoDiaSelecionado` ou numa célula do calendário com registro (`navigation.navigate('Detalhes', { registroId })`) | `TelaFormulario` (modo edição), `TelaDetalhes`    |
...
| Confirmação de exclusão                             | Estado local (`modalVisivel`, `useState`) + componente próprio `ModalConfirmacao` (não usa `Alert.alert`, que não funciona no Expo Web) | Disparado pelo `onPress` do `BotaoExcluir`                                                                                       | `TelaFormulario`, `TelaDetalhes`                  |
